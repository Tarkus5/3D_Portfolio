    ...
    return (
        <div className='absolute top-10 left-10 right-0 flex justify-center items-center'>
            <div
                ...
                role='alert'
            >
                <p className={`${type === 'danger' ? 'bg-red-500': 'bg-blue-500'} flex rounded-full uppercase px-2 py-1 font-semibold mr-3 text-xs`}>{ type === 'danger' ? `${ t("Alert.failed") }` : `${ t("Alert.success") }`}</p>
